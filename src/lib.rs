...
fn angle_encode_batch_simd<'py>(
    ...
    let data_slice = batch_data.as_slice().unwrap();
    
    // Process each batch
    for b in 0..batch_size {
        // Create a slice for this batch
        let start = b * data_dim;
        let end = start + data_dim;
        let batch_slice = &data_slice[start..end];
...
#[pymodule]
fn _simd_angle_encoder(_py: Python, m: &PyModule) -> PyResult<()> {
